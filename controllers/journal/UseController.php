...
class UseController extends Controller
{
	...
	public function actionRunaction() {
		...
		if(count($id) > 0) {
			...
			$criteria->addInCondition('use_id', $id);

			if($actions == 'publish') {
				ArticleJournalUse::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				ArticleJournalUse::model()->deleteAll($criteria);
			}
		}
	...
	{
		$model = ArticleJournalUse::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404, Yii::t('phrase', 'The requested page does not exist.'));
		return $model;
	}
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='article-journal-use-form') {
