...
class FileController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = ArticleJournalFile::model()->findByPk(1, array(
		...
		$dataProvider = new CActiveDataProvider('ArticleJournalFile', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Article Journal Files');
	...
	public function actionView($id) 
	{
		...
		$this->pageTitle = Yii::t('phrase', 'View Article Journal Files');
	...
	{
		$model=new ArticleJournalFile;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['ArticleJournalFile'])) {
			$model->attributes=$_POST['ArticleJournalFile'];
			...
			*/
			...
			if(Yii::app()->getRequest()->getParam('enablesave') == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'ArticleJournalFile success created.'));
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['ArticleJournalFile'])) {
			...
			*/
			...
			if(Yii::app()->getRequest()->getParam('enablesave') == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'ArticleJournalFile success updated.'));
					//$this->redirect(array('view','id'=>$model->file_id));
					$this->redirect(array('manage'));
				}
			}
	...
	public function actionRunaction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				ArticleJournalFile::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				ArticleJournalFile::model()->deleteAll($criteria);
			}
		}
	...
	{
		$model = ArticleJournalFile::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404, Yii::t('phrase', 'The requested page does not exist.'));
		return $model;
	}
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='article-journal-file-form') {
